...
describe("SettingsView semantic theme", () => {
  beforeEach(() => {
    ...
    invokeMock.mockImplementation((command: string) => {
      ...
      if (command === "list_provider_models") return Promise.resolve(["qwen-vl-max", "qwen-vl-plus"]);
  ...
  test("uses semantic classes and keeps regular-user tabs focused", async () => {
    ...
    expect(screen.getByRole("button", { name: "高级配置：图片理解路由" })).toBeInTheDocument();
  ...
  test("shows the vision routing editor with image-understanding labels", async () => {
    render(<SettingsView onClose={() => {}} initialTab="capabilities" />);

    await waitFor(() => {
      expect(screen.getByText("能力类型")).toBeInTheDocument();
    });

    expect(screen.getByRole("option", { name: "图片理解 Vision" })).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(screen.getByText("上传图片、截图或其他需要识图的请求，会走这里配置的“图片理解 Vision”主模型和回退链。")).toBeInTheDocument();
      expect(screen.getByText("图片理解主模型")).toBeInTheDocument();
    });
  });

  test("auto-selects the first vision model after choosing a vision provider", async () => {
    invokeMock.mockImplementation((command: string) => {
    ...
    fireEvent.change(selects[0] as HTMLSelectElement, { target: { value: "vision" } });

    await waitFor(() => {
      expect(screen.getByText("图片理解主连接")).toBeInTheDocument();
    });

    const providerSelect = screen.getAllByRole("combobox")[1] as HTMLSelectElement;
    fireEvent.change(providerSelect, { target: { value: "provider-vision" } });

    await waitFor(() => {
      expect(screen.getByDisplayValue("qwen-vl-max")).toBeInTheDocument();
    });
  });

  test("shows only vision-enabled connections in the vision routing provider list", async () => {
    invokeMock.mockImplementation((command: string) => {
      if (command === "list_model_configs") {
        return Promise.resolve([
          {
      ...
      if (command === "list_provider_models") return Promise.resolve(["qwen-vl-max"]);
      if (command === "get_capability_routing_policy") return Promise.resolve(null);
      if (command === "list_capability_route_templates") return Promise.resolve([]);
      if (command === "get_feishu_gateway_settings") {
        return Promise.resolve({
    ...
    const providerSelect = screen.getAllByRole("combobox")[1] as HTMLSelectElement;
    const optionTexts = Array.from(providerSelect.options).map((option) => option.textContent);

    expect(optionTexts).toContain("Qwen Vision");
    expect(optionTexts).not.toContain("DeepSeek Chat");
  });

  test("falls back to the configured model name when a custom vision connection has no recommended model list", async () => {
    invokeMock.mockImplementation((command: string) => {
      if (command === "list_model_configs") {
        return Promise.resolve([
          {
            id: "provider-custom-vision",
            name: "Custom Vision",
            api_format: "openai",
            base_url: "http://111.51.78.135:8060/v1",
      ...
      if (command === "list_provider_models") return Promise.resolve([]);
      if (command === "get_capability_routing_policy") return Promise.resolve(null);
      if (command === "list_capability_route_templates") return Promise.resolve([]);
      if (command === "get_feishu_gateway_settings") {
        return Promise.resolve({
    ...
    const providerSelect = screen.getAllByRole("combobox")[1] as HTMLSelectElement;
    fireEvent.change(providerSelect, { target: { value: "provider-custom-vision" } });

    await waitFor(() => {
      expect(screen.getByDisplayValue("Qwen3-VL-32B-Instruct")).toBeInTheDocument();
    });
  });
