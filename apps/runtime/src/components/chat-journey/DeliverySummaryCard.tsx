...
export function DeliverySummaryCard({
  ...
  return (
    <div className="max-w-[80%] rounded-2xl border border-emerald-200 bg-emerald-50/80 px-5 py-4 text-sm text-emerald-950 shadow-sm">
      <div className="text-xs font-medium text-emerald-700">交付结果</div>
      <div className="mt-1 text-base font-semibold">
        {model.status === "partial" ? "已生成部分产物" : model.status === "failed" ? "交付失败" : "已生成交付产物"}
      </div>
      <div className="mt-2 text-xs text-emerald-800">{summaryText}</div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {model.deliverables.length > 0 && onViewFiles && (
          <button
            type="button"
            onClick={onViewFiles}
            className="rounded-lg border border-emerald-200 bg-white px-3 py-1.5 text-xs font-medium text-emerald-800 hover:bg-emerald-50"
          >
            查看文件
          </button>
        )}
      ...
      {model.deliverables.length > 0 && (
        <div className="mt-4 space-y-2">
          {model.deliverables.map((item) => (
            <div key={item.path} className="rounded-xl border border-emerald-100 bg-white/90 px-3 py-2">
